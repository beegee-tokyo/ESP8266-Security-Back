...
void triggerLights() {
	if (lightIp[0] == 0) { // If 0, no lightIp was stored
		if (debugOn) {
			sendDebug("triggerLights - no light IP found", OTA_HOST);
		}
		return;
	}
	comLedFlashStart(0.1);
	...
	if (!tcpClient.connect(lightIp, tcpComPort)) {
		Serial.println("connection to backyard lights " + String(lightIp[0]) + "." + String(lightIp[1]) + "." + String(lightIp[2]) + "." + String(lightIp[3]) + " failed");
		tcpClient.stop();
		comLedFlashStop();
		return;
	}
